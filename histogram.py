...
# ALT + MAJ + E: execute une ligne de code
...
cv2.normalize(hist2, hist2, norm_type=cv2.NORM_L2)# compris entre 0 et 1
...
# afficher son histogram
plt.plot(hist2,color = 'r')
plt.xlim([0,256])
plt.show()

# distance entre box et lena
dist=cv2.norm(hist, hist2, normType=cv2.NORM_L2, mask=None)
print("Distance between h1 and h2 = %.9f" % dist)#0.687690110
...
print( "3D histogram shape: %s, with %d values" % (colhist.shape, colhist.flatten().shape[0]))
...
# 4. Compare with the distance between color histograms of 'lena' and 'baboon'
